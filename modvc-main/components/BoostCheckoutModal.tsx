...
interface BoostCheckoutModalProps {
  ...
  onSuccess?: (numberOfBoosts: number) => void;
}

export function BoostCheckoutModal({ isOpen, onClose, server, currentUser, masterWalletAddress, onSuccess }: BoostCheckoutModalProps) {
  const [isProcessing, setIsProcessing] = useState(false);
  const [quantity, setQuantity] = useState(1);
  const [showTerms, setShowTerms] = useState(false);
  const PRICE_PER_BOOST = 3.00;
    ...
    try {
      ...
      if (data.success && data.url) {
        if (newWindow) {
           ...
           toast.success("Checkout opened in a new tab! This page will update automatically.");
           onClose();
        } else {
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
      <div className="bg-[#313338] rounded-md w-full max-w-[440px] overflow-hidden shadow-2xl flex flex-col animate-in fade-in zoom-in duration-200">
        ...
        <div className="p-4 flex-1 overflow-y-auto custom-scrollbar max-h-[70vh]">
          ...
          <div className="bg-fuchsia-500/10 border border-fuchsia-500/20 rounded-md p-4 mb-6 flex items-start gap-3">
             <div className="w-10 h-10 rounded-full bg-fuchsia-500/20 flex items-center justify-center shrink-0">
               <Zap className="w-5 h-5 text-fuchsia-400" />
             </div>
             <div>
               <h3 className="text-sm font-bold text-white mb-1">Unlock Server Perks!</h3>
               <p className="text-xs text-gray-300 leading-relaxed">
                 Boosting <strong className="text-white">{server.name}</strong> helps the community grow. 
                 <strong className="text-fuchsia-400"> 100%</strong> of this payment goes directly to the server owner via Stripe!
               </p>
             </div>
          </div>

          {/* Quantity Selector */}
          <div className="mb-6 bg-[#2B2D31] rounded-md p-3 border border-[#1e1f22] flex items-center justify-between">
            <div>
              <div className="text-sm font-bold text-white mb-0.5">Number of Boosts</div>
              <div className="text-xs text-gray-400">${PRICE_PER_BOOST.toFixed(2)} USD per boost</div>
            </div>
            <div className="flex items-center bg-[#1e1f22] rounded overflow-hidden border border-white/5">
               <button 
               ...
               <div className="px-3 py-1.5 font-bold text-white min-w-[40px] text-center border-x border-white/5">
                 {quantity}
               </div>
               <button 
          ...
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-[15px] font-bold text-white">${(quantity * PRICE_PER_BOOST).toFixed(2)} USD</span>
          </div>
          ...
          {!showTerms ? (
            ...
            <div className="mt-2 bg-[#1e1f22] p-3 rounded-md border border-white/5 animate-in fade-in slide-in-from-top-2 duration-200">
              ...
              <ul className="text-[10px] text-gray-400 leading-relaxed list-disc pl-4 space-y-1">
                ...
                <li>Boosts are non-refundable and last for the duration specified by the server owner.</li>
              </ul>
            </div>
          )}
